...
class DGFE(nn.Module):   
    def __init__(self, gate_channels, no_spatial=False, num_feature_levels=4):
    ...
    def forward(self, x, memory, spatial_shapes):  
        ...
        return x_out
@register()
class DGDQDFINETransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.LDMG = DH(self.hidden_dim, feat_strides[0], kernel_sizes=[3, 5, 7, 9, 11])
        self.multiscale = MultiScaleFeature(self.hidden_dim, scale=self.num_levels) 
        self.DGFE = DGFE(gate_channels=self.hidden_dim, num_feature_levels=self.num_levels)
    ...
    def _select_topk(self, memory: torch.Tensor, outputs_logits: torch.Tensor, densityMapMemory: torch.Tensor, outputs_anchors_unact: torch.Tensor, topk: int): 
        ...
        if self.query_select_method == 'default':
            if self.iter < self.using_densitymap_iter:  
                ...
                # ----------------------------
                score = outputs_logits.max(-1).values * densityMapMemory
                score = torch.where(score < self.score_thresh, score * 0.1, score)
