...
class CoordAtt_1(nn.Module):
    def __init__(self, inp, reduction=32):
        super(CoordAtt_1, self).__init__()
        ...
        self.dwconv = nn.Conv2d(inp, inp, 3, 1, 1, groups=inp)
...
class CoordAtt(nn.Module):
    def __init__(self, inp, reduction=2):
...
class CAFM_1(nn.Module):  
    def __init__(self, out_channels=[256, 256]):
        super(CAFM_1, self).__init__()

        self.se1 = ChannelAttention(out_channels[0])
        self.se2 = ChannelAttention(out_channels[1])
        self.sa =  CoordAtt(out_channels[0])
    ...
    def forward(self, x):  
        ...
        return out
class SGLPBlock(nn.Module):
    def __init__(self, inc, ouc, height, width, global_ratio=0.25, local_ratio=0.25) -> None:
        super().__init__()
        ...
        self.global_branch = DAttention(self.global_channels, (self.height, self.width), n_heads=1)
        self.local_branch = CC(self.local_channels, self.local_channels, k=3, s=1, num_experts=2)
...
class SGLPBlock(nn.Module):
    def __init__(self, inc, ouc, n_div=4):
        ...
        self.partial_conv = CC(self.partial_channels, self.partial_channels, k=3, s=1, num_experts=2)
    ...
    def forward(self, x):
        ...
        return y
class CC_Single(nn.Module):
    """ Dynamic Conv layerCC
    """
    def __init__(self, in_features, out_features, kernel_size=1, stride=1, padding='', dilation=1,   
                 groups=1, bias=False, num_experts=4, eps=1e-6):     
        super().__init__()
        self.eps=eps
        self.dconv=nn.Conv2d(in_features, in_features, 3, stride=1, padding=1, groups=in_features, bias=False)
        self.pconv=nn.Conv2d(in_features, in_features, 1, 1, 0, 1, bias=False)    
        self.routing = nn.Linear(in_features, num_experts)
        self.cond_conv = CondConv(in_features, out_features, kernel_size, stride, padding, dilation,
    ...
    def forward(self, x):
        ...
        routing_weights = routing_weights / (routing_weights.sum(dim=1, keepdim=True) + self.eps)  
...
class CC(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, num_experts=4): 
        super().__init__()     
        self.conv = nn.Sequential(
            CC_Single(c1, c2, kernel_size=k, stride=s, padding=autopad(k, p, d), dilation=d, groups=g, num_experts=num_experts),
            nn.BatchNorm2d(c2),
